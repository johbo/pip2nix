...
def render_packages(packages, include_lic, cache):
    return '\n'.join(
        '"{}" = {}'.format(pkg.name,
                           pkg.to_nix(include_lic=include_lic, cache=cache))
...
def read_hash_cache(path):
    """
    Recover the url to hash mapping of a previously generated file, so
    that known sources do not have to be prefetched again.
    """
