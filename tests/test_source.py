...
def test_keeps_the_revision_of_a_git_url():
    source = Source.from_url('git+https://git.example/repo@main')
    assert source.scheme == 'git+https'
    assert source.url == 'git+https://git.example/repo@main'
