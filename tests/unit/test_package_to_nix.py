...
def test_renders_a_license_the_hand_written_map_knows():
    ...
    assert ('license = [ pkgs.lib.licenses.gpl3 ];'
            in package.to_nix(include_lic=True))
...
def test_renders_each_license_once(nix_licenses):
    ...
    assert ('license = [ pkgs.lib.licenses.mit ];'
            in package.to_nix(include_lic=True))


def test_renders_the_first_license_by_name_when_nixpkgs_knows_none(
        nix_licenses):
    package = make_package(WHEEL_URL,
                           licenses=['Frobnicate 1.0', 'Frobnicate 2.0'])

    assert ('license = [ { fullName = "Frobnicate 1.0"; } ];'
            in package.to_nix(include_lic=True))
